...
export default function PersonalRecordsModal({ isOpen, onClose, workouts }: PersonalRecordsModalProps) {
  ...
  return (
    <div
      ...
      tabIndex={-1}
    >
      <div className="relative w-full max-w-5xl max-h-[90vh] bg-gradient-to-br from-slate-900 to-slate-800 rounded-2xl shadow-2xl border border-slate-700 flex flex-col">
        {/* Header */}
        <div className="sticky top-0 z-10 bg-slate-900/95 backdrop-blur-sm border-b border-slate-700 p-6 rounded-t-2xl">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-2xl font-bold text-white">Todos tus Récords Personales</h2>
                <p className="text-sm text-slate-400">
                  {stats.total} ejercicios · Promedio: {stats.avgWeight} kg · Máximo: {stats.maxPR} kg
                </p>
              </div>
            </div>
            <button
              ...
              <table className="w-full">
                <thead className="sticky top-0 bg-slate-800/95 backdrop-blur-sm">
                  <tr className="text-left border-b border-slate-700">
                    <th className="px-4 py-3 text-sm font-semibold text-slate-300">#</th>
                    <th className="px-4 py-3 text-sm font-semibold text-slate-300">Ejercicio</th>
                    <th className="px-4 py-3 text-sm font-semibold text-slate-300 text-right">Peso Máximo</th>
                    <th className="px-4 py-3 text-sm font-semibold text-slate-300 text-right">Reps Máximas</th>
                    <th className="px-4 py-3 text-sm font-semibold text-slate-300 text-right">Fecha</th>
                  </tr>
                </thead>
                <tbody>
                  {filteredPRs.map((pr, index) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-4 text-sm font-medium text-white">{pr.exerciseName}</td>
        ...
        {/* Footer */}
        <div className="sticky bottom-0 bg-slate-900/95 backdrop-blur-sm border-t border-slate-700 p-4 rounded-b-2xl">
          <div className="flex items-center justify-between text-sm">
            <p className="text-slate-400">
              Mostrando {filteredPRs.length} de {stats.total} récords
            </p>
            <button
              onClick={handleClose}
